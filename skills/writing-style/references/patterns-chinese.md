...
本目录源自维基百科 AI 清理指南，原生体裁是百科条目。下面所有示例的"改写后"都比"改写前"短，但短是副产品：这些示例删掉的是零信息套话，同时把模糊说法换成了具体事实。使用时守住三条：
...
**改写后**：
> API 网关把每个请求转发到对应的服务，并拦截认证失败的请求。
...
**改写后**：
> 对称布局通常让用户觉得更可预测。团队过度优化流程时，容易忽略人的实际使用方式。
