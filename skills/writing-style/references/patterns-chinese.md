# 中文 AI 写作模式 - 完整参考

Complete Chinese AI Writing Patterns Reference.

## 使用判据（先读这节再用目录）

本目录源自维基百科 AI 清理指南，原生体裁是百科条目。模式用于定位实际文本中的表达问题；示例修改保留输入已给的信息，原文没有实质内容时删除套话，需要补充语境时明确缺口。使用时守住三条：

- 改写方向是"换具体"或"删零信息"，不是"变短"。信息守恒判据（删掉后读者少知道了什么，见 `chinese-writing.md` 总判据）先于任何模式匹配。
- 示例里"补具体事实"的动作（换成调查年份、机构名、具体数字）只有在原文或上下文已提供该事实时才可做；没有可换的事实时保留原有信息，只调表述，不发明新事实。
- 技术笔记、解释性文档里承担因果推理的句子不是套话。命中模式表面形式但承载真实信息的句子，改句形不删内容。

### 反例一：形似模式但承载信息，不删

**原文**（技术笔记，形似"通用积极结论"）：
> 开销不会凭空消失，只是从 token dropping 转嫁到了显存、通信和调度上。

**错误处理**：判为评价性收束句删除。
**正确处理**：保留。这是本节的结论，删掉后读者不知道 dropless 方案的代价去了哪里。

**原文**（形似"否定式排比"）：
> Token dropping 通常不是把整个 token 从模型里删掉，而是这个 token 在当前层没拿到原定的专家计算。

**错误处理**：按反向句式改写或删除。
**正确处理**：保留。"删掉 token"是读者真实会有的误读，这个对比在消解具体歧义，不在自我定位。

### 反例二：改对但没变短

**改写前**：
> 该机制有效提升了系统的整体性能，为后续优化奠定了坚实基础。

**改写后**（数字与因果来自上下文已有内容）：
> 开启该机制后，路由不均衡度从 0.31 降到 0.12，为后面把专家数从 8 扩到 64 留出了余量。

改写后更长。修复方向是把空泛评价换成上下文已有的具体数字与因果，不是把两句砍成一句。

---

## 内容模式 (CONTENT PATTERNS)

### 1. 过度强调意义、遗产和更广泛的趋势

**需要注意的词汇**：作为/充当、标志着/见证、是……的体现/证明/提醒、极其重要的/重要的/至关重要的/核心的/关键性的作用/时刻、凸显/强调/彰显了其重要性/意义、反映了更广泛的、象征着其持续的/永恒的/持久的、为……做出贡献、为……奠定基础、标志着/塑造着、代表/标志着一个转变、关键转折点、不断演变的格局、焦点、不可磨灭的印记、深深植根于

**改写前**：
> 加泰罗尼亚统计局于 1989 年正式成立，标志着西班牙区域统计演变史上的关键时刻。这一举措是西班牙全国范围内更广泛运动的一部分，旨在分散行政职能并加强区域治理。

**改写后**：
> 加泰罗尼亚统计局成立于 1989 年，是西班牙分散行政职能、加强区域治理的举措之一。

---

### 2. 过度强调知名度和媒体报道

**需要注意的词汇**：独立报道、地方/区域/国家媒体、由知名专家撰写、活跃的社交媒体账号

**改写前**：
> 她的观点被《纽约时报》、BBC、《金融时报》和《印度教徒报》引用。她在社交媒体上拥有活跃的存在，拥有超过 50 万粉丝。

**改写后**：
> 她的观点被《纽约时报》、BBC、《金融时报》和《印度教徒报》引用。她活跃于社交媒体，拥有超过 50 万粉丝。

---

### 3. 以 -ing 结尾的肤浅分析

**需要注意的词汇**：突出/强调/彰显……、确保……、反映/象征……、为……做出贡献、培养/促进……、涵盖……、展示……

**改写前**：
> 寺庙的蓝色、绿色和金色色调与该地区的自然美景产生共鸣，象征着德克萨斯州的蓝帽花、墨西哥湾和多样化的德克萨斯州景观，反映了社区与土地的深厚联系。

**改写后**：
> 寺庙的蓝色、绿色和金色象征着德克萨斯州的蓝帽花、墨西哥湾和当地其他景观。

---

### 4. 宣传和广告式语言

**需要注意的词汇**：拥有（夸张用法）、充满活力的、丰富的（比喻）、深刻的、增强其、展示、体现、致力于、自然之美、坐落于、位于……的中心、开创性的（比喻）、著名的、令人叹为观止的、必游之地、迷人的、把关、保真度

**改写前**：
> 位于托斯卡纳令人叹为观止的区域内，这座村庄充满活力，拥有丰富的文化遗产和迷人的自然美景。

**改写后**：
> 村庄位于托斯卡纳，具有当地的文化遗产与自然景观。

---

### 5. 模糊归因和含糊措辞

**需要注意的词汇**：行业报告显示、观察者指出、专家认为、一些批评者认为、多个来源/出版物（实际引用却很少）

**改写前**：
> 由于其独特的特征，浩来河引起了研究人员和保护主义者的兴趣。专家认为它在区域生态系统中发挥着至关重要的作用。

**改写后**：
> 浩来河引起了研究人员和保护工作者的兴趣，专家认为它对区域生态系统很重要。

专家身份与证据仍需补实，给定文本不足以确定具体机构或调查结果。

---

### 6. 提纲式的"挑战与未来展望"部分

**需要注意的词汇**：尽管其……面临若干挑战……、尽管存在这些挑战、挑战与遗产、未来展望

**改写前**：
> 尽管工业繁荣，Korattur 面临着城市地区典型的挑战，包括交通拥堵和水资源短缺。尽管存在这些挑战，凭借其战略位置和正在进行的举措，Korattur 继续蓬勃发展，成为钦奈增长不可或缺的一部分。

**改写后**：
> Korattur 的工业繁荣，同时面临交通拥堵和水资源短缺。

---

## 语言和语法模式 (LANGUAGE PATTERNS)

### 7. 过度使用的"AI 词汇"

**高频 AI 词汇**：此外、与……保持一致、至关重要、深入探讨、强调、持久的、增强、培养、获得、突出（动词）、相互作用、复杂/复杂性、关键（形容词）、格局（抽象名词）、关键性的、展示、织锦（抽象名词）、证明、强调（动词）、宝贵的、充满活力的

**改写前**：
> 此外，索马里菜肴的一个显著特征是加入骆驼肉。意大利殖民影响的持久证明是当地烹饪格局中广泛采用意大利面，展示了这些菜肴如何融入传统饮食。

**改写后**：
> 索马里菜肴包括骆驼肉，意大利面也很常见，体现了意大利殖民的影响。

---

### 8. 避免使用"是"（系动词回避）

**需要注意的词汇**：作为/代表/标志着/充当 [一个]、拥有/设有/提供 [一个]

**改写前**：
> Gallery 825 作为 LAAA 的当代艺术展览空间。画廊设有四个独立空间，拥有超过 3000 平方英尺。

**改写后**：
> Gallery 825 是 LAAA 的当代艺术展览空间。画廊有四个独立空间，总面积超过 3000 平方英尺。

---

### 9. 否定式排比

**改写前**：
> 这不仅仅是节拍在人声下流动；它是攻击性和氛围的一部分。这不仅仅是一首歌，而是一种声明。

**改写后**：
> 人声下的节拍加强了歌曲的攻击性氛围。

---

### 10. 三段式法则过度使用

**改写前**：
> 活动包括主题演讲、小组讨论和社交机会。与会者可以期待创新、灵感和行业洞察。

**改写后**：
> 活动包括主题演讲、小组讨论和社交机会。

---

### 11. 刻意换词（同义词循环）

**改写前**：
> 主人公面临许多挑战。主要角色必须克服障碍。中心人物最终获得胜利。英雄回到家中。

**改写后**：
> 主人公面临许多挑战，但最终获得胜利并回到家中。

---

### 12. 虚假范围

**改写前**：
> 我们穿越宇宙的旅程将我们从大爆炸的奇点带到宏伟的宇宙网，从恒星的诞生和死亡到暗物质的神秘舞蹈。

**改写后**：
> 我们讨论大爆炸、宇宙网、恒星的诞生与死亡，以及暗物质。

---

## 风格模式 (STYLE PATTERNS)

### 13. 破折号过度使用

**改写前**：
> 这个术语主要由荷兰机构推广——而不是由人民自己。你不会说"荷兰，欧洲"作为地址——但这种错误标记仍在继续——即使在官方文件中。

**改写后**：
> 这个术语主要由荷兰机构推广，而不是由人民自己。你不会说"荷兰，欧洲"作为地址，但这种错误标记在官方文件中仍在继续。

---

### 14. 粗体过度使用

**改写前**：
> 它融合了 **OKR（目标和关键结果）**、**KPI（关键绩效指标）** 和视觉战略工具，如 **商业模式画布（BMC）** 和 **平衡计分卡（BSC）**。

**改写后**：
> 它融合了 OKR（目标和关键结果）、KPI（关键绩效指标）和视觉战略工具，如商业模式画布（BMC）和平衡计分卡（BSC）。

---

### 15. 内联标题垂直列表

**改写前**：
> - **用户体验：** 用户体验通过新界面得到显著改善。
> - **性能：** 性能通过优化算法得到增强。
> - **安全性：** 安全性通过端到端加密得到加强。

**改写后**：
> 更新改进了界面与算法性能，并通过端到端加密加强安全性。

---

## 交流模式 (COMMUNICATION PATTERNS)

### 16. 协作交流痕迹

**需要注意的词汇**：希望这对您有帮助、当然！、一定！、您说得完全正确！、您想要……、请告诉我、这是一个……

**改写前**：
> 这是法国大革命的概述。希望这对您有帮助！如果您想让我扩展任何部分，请告诉我。

**处理**：
删除这段对话套话。输入没有提供可供改写的历史内容。

---

### 17. 知识截止日期免责声明

**需要注意的词汇**：截至 [日期]、根据我最后的训练更新、虽然具体细节有限/稀缺……、基于可用信息……

**改写前**：
> 虽然关于公司成立的具体细节在现成资料中没有广泛记录，但它似乎是在 20 世纪 90 年代的某个时候成立的。

**改写后**：
> 该公司似乎成立于 20 世纪 90 年代，现有资料没有给出确切年份。

---

## 填充模式 (FILLER PATTERNS)

### 18. 填充短语

| 改写前 | 改写后 |
|--------|--------|
| "为了实现这一目标" | "为了实现这一点" |
| "由于下雨的事实" | "因为下雨" |
| "在这个时间点" | "现在" |
| "在您需要帮助的情况下" | "如果您需要帮助" |
| "系统具有处理的能力" | "系统可以处理" |
| "值得注意的是数据显示" | "数据显示" |

---

### 19. 过度限定

**改写前**：
> 可以潜在地可能被认为该政策可能会对结果产生一些影响。

**改写后**：
> 该政策可能会影响结果。

---

### 20. 通用积极结论

**改写前**：
> 公司的未来看起来光明。激动人心的时代即将到来，他们继续追求卓越的旅程。这代表了向正确方向迈出的重要一步。

**处理**：
删除这段空泛评价。输入没有提供具体的未来计划。

---

## 来源

[基于维基百科：AI 写作特征](https://en.wikipedia.org/wiki/Wikipedia/Signs_of_AI_writing)

---

## 比喻滥用模式（非维基补充）

### 条件反射式比喻

**需要注意的写法**：把概念比作引擎、桥梁、地图、旅程、食谱、生态系统、飞轮、罗盘、镜子、工具箱，而比喻本身不增加任何理解。

**改写前**：
> 可以把 API 网关想象成指挥交通的警察，它是各服务之间的桥梁，是整个架构跳动的心脏。

**改写后**：
> API 网关在服务之间转发请求。

---

## 节奏与修辞补充（非维基）

### 连续短句制造戏剧感

**需要注意的写法**：连续三个以上短促断句渲染气势，常配"没有 X。没有 Y。只有 Z。"式的排比省略句。

**问题**：单个短句可以强调重点；连续短句把普通信息包装成宣传片台词，信息量没有增加。

**改写前**：
> 随后新版本上线了。没有公告。没有迁移文档。旧接口直接失效了。

**改写后**：
> 新版本上线时没有发布公告和迁移文档，旧接口直接失效。

### 格言与金句公式

**需要注意的结构**："X 是 Y 的语言 / 货币"、"X 不是工具而是镜子"、"X 的底层是 Y"、"X 成了陷阱"。

**问题**：把普通主张包装成可引用的格言，姿态提高了，精度没有提高。

**改写前**：
> 对称是信任的语言。效率一旦被过度追求，就会变成陷阱。

**处理**：
根据已有语境确定“对称”“信任”和“效率”所指的对象，再写清实际关系。语境不足时指出歧义，不能补出原文没有的心理效应或失败原因。

### 假装坦率的修辞开场

**需要注意的词汇**：说实话、老实讲、坦白说、讲真、事情是这样的、听我说。

**问题**：先用一句坦率姿态的钩子制造停顿，随后给出的仍是普通答案。这些词在口语记录里自然出现不算问题，成为固定开场公式才需要处理。

**改写前**：
> 它值这个价吗？说实话？取决于你多久用一次。

**改写后**：
> 它是否值这个价，取决于你的使用频率。
