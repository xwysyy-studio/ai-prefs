...
- Substantial prose → `~/.claude/rules/writing-tone.md`。常驻最低限：破折号（`——` `—`）全面禁止；被用户纠正过的术语绝不再用；删掉后读者没少知道什么的句子不写；只回应已有文本证据提出的问题，不主动制造误解、质疑、替代定位与边界澄清；对象、范围和贡献正面陈述，严禁"不是 / 并非 X，而是 Y"及其语义等价改写，相关工作比较从对方的覆盖不足切入；概念用用户既定说法或目标读者领域的通用术语，内部命名、缩写、项目编号、流程标签、状态码和比喻不代替事实说明，确无通用说法才命名并在首次出现时解释；能用直接写法写清对象、动作、条件和结果且更准确时，只保留直接写法。
...
- Web retrieval 按需要的东西选通路，不按顺手：论文检索与元数据 → ai4scholar（`curl`，config `~/.agents/configs/ai4scholar.json`），DBLP / CrossRef / arXiv 兜底；公开网页发现、已知 URL 正文抓取、多来源调研（`find --sweep` 并行横扫社交平台/网页/新闻）优先使用 `find-and-fetch` skill，内置 Web Search 仍可按任务选择；事实核查先定位权威来源再抓取原页，search 摘要不单独作为证据；GitHub → `gh` CLI，不用 curl 打 api.github.com。`browser-use` 只用于任务本身需要交互、登录或会话状态、浏览器布局或截图的场景，公开页面依赖 JavaScript 渲染不构成启动浏览器的理由；普通资料检索不经本地浏览器的搜索引擎（本机实测：本地浏览器搜索被地域重定向到国内版引擎并返回 SEO 内容农场，云端检索不受影响）。所选通路失败时报告失败，不静默改走另一条。
