...
- Non-trivial dev work → `~/.claude/rules/dev-core.md`（语义确认 / 编码克制 / 写完后 / 并发 / subagent 契约 / API 脚本与失败阶梯）。开发常驻最低限（不读全文也生效）：
  ...
  - diff 里的注释、文档、commit / PR 文案按"读者没有读过本对话"检查：回应对象只存在于会话史的解释句（为什么不用 X / 改用 Y / 与旧版兼容）清理，改动叙事写进交回报告；本对话中被纠正过的文本段落按终态整段重写，不在旧句上打补丁。
...
- Substantial prose → `~/.claude/rules/writing-tone.md`。常驻最低限：破折号（`——` `—`）全面禁止；被用户纠正过的术语绝不再用；删掉后读者没少知道什么的句子不写；产物按"读者没有读过本对话"写，否定、澄清与解释的回应对象只存在于会话史的（被否决方案、产物旧版本、某次纠正）不写，改动叙事进交回报告；纠正一经确认转写为只写现行要求的正向终态口径，此后只引用终态口径；被纠正过的文本定稿按终态内容清单整段重写并核对信息不丢，不在旧句上打补丁；只回应已有文本证据提出的问题，不主动制造误解、质疑、替代定位与边界澄清；对象、范围和贡献正面陈述，严禁"不是 / 并非 X，而是 Y"及其语义等价改写，相关工作比较从对方的覆盖不足切入；概念用用户既定说法或目标读者领域的通用术语，内部命名、缩写、项目编号、流程标签、状态码和比喻不代替事实说明，确无通用说法才命名并在首次出现时解释；能用直接写法写清对象、动作、条件和结果且更准确时，只保留直接写法。
...
- Web retrieval 按需要的东西选通路，不按顺手：论文检索与元数据 → ai4scholar（`curl`，config `~/.agents/configs/ai4scholar.json`），DBLP / CrossRef / arXiv 兜底，arXiv 论文一律拉 e-print 源码包读 tex，不用 HTML 版（丢表格数值与未展开宏）；公开网页发现、已知 URL 正文抓取、多来源调研（`--source` 按任务多选通道并行：web / 新闻 / 知乎 / 小红书 / 推特）优先使用 `find-and-fetch` skill，内置 Web Search 仍可按任务选择；事实核查先定位权威来源再抓取原页，search 摘要不单独作为证据；GitHub → `gh` CLI，不用 curl 打 api.github.com。`browser-use` 用于任务本身需要交互、登录或会话状态、浏览器布局或截图的场景，也用于结构化来源未提供且任务实际需要的站点原生全文索引、相关性排序、专用筛选或交互式结果探索；arXiv 的网页全文检索和网页相关性排序直接走这条路径，命中这些语义即可直接使用 Browser Use，无需等待其他通路失败。公开页面依赖 JavaScript 渲染不构成启动浏览器的理由；普通资料检索不经本地浏览器的搜索引擎（本机实测：本地浏览器搜索被地域重定向到国内版引擎并返回 SEO 内容农场，云端检索不受影响）。所选通路失败时报告失败，不静默改走另一条。
