...
- 过度工程扫描：diff 新增行里的 try / fallback / 新抽象 / config 开关 / 防御分支，每处指不出真实失败场景 → 撤回。外部审查返回的加固建议按同一判据过滤，不采纳记一句理由；自己当审查者时同一判据管输出侧：审查 = 找有真实失败场景的 bug 和设计层缺陷（方案没经过真实处境推演、在真实处境下站不住，同样是缺陷），不产出防御性加固清单
