...
十条 Core Rules 都有真实摩擦史背书并全域生效；Claude Code 每次会话自动加载 `rules/*.md`，Codex 的对应消费点见文末表。
...
- 反馈归置与记录门槛：用户的纠正与偏好默认按通用吸收，通用规则落全局（CLAUDE.md / rules），绑定具体项目对象的事实、裁定与防复活记录才进 memory，可复用技术方法记 memory reference；在 memory 里发现通用规则，随手上移全局。记录门槛三条全过才写：换个项目、过几个月还有用；无法从代码 / git / 现有规则推导；指得出它会改变未来哪个具体行为。不记已固化进规则的转述、执行流水与调查过程史，只留终态结论、防复活裁定与恢复入口，过程史靠 transcript 引擎回捞。

# 深度细则（Claude Code 自动加载，各文件是唯一权威）
...
| Codex / GPT-6 Astra 委派 | `references/codex-delegation.md`、`references/codex-task-template.md` |
