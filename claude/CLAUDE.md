...
通用偏好进入全局规则；项目事实、裁定和防复活记录进入项目现有文档或 memory，可复用技术方法进入对应 reference。只记录跨时间仍有用、无法从现有代码或规则推导、会改变具体后续行动的内容；不复制调查流水与已固化规则。文件写入仍遵守本任务授权。非 Git 配置修改前做时间戳备份，检查真实差异与消费者行为。
